...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        int nge = -1;   // default value if no greater element is found

        // Look to the right of arr[i]
        for (int j = i + 1; j < n; j++) {

            if (arr[j] > arr[i]) {
                nge = arr[j];    // first greater element on the right
                break;
            }
        }
        ...
        if (i == n - 1)
            printf("%d", nge);      // last element (no trailing comma)
        else
            printf("%d, ", nge);
    }
